...
echo "[setup] preflight inside $RUNTIME_IMAGE (hf/huggingface_hub + decon)..."
docker run --rm -w /workspace "$RUNTIME_IMAGE" bash -lc \
  'command -v hf && python -c "import huggingface_hub; print(\"huggingface_hub\", huggingface_hub.__version__)" && decon/bin/decon --version'
...
exec uv run eval @ configs/eval/deepseek-v4-pro-400M-300turn-codex.toml
