...
from pretrain_curation_gym.util.hf_access import (
    DatasetAccessError,
    FetchKey,
...
from pretrain_curation_gym.util.utils import extract_hf_commands
...
from .conftest import NoOpLeakageDetector, bind_fast_scorer
...
from pretrain_curation_gym.gpu.train_gpt import GPT2_SMALL_PARAM_COUNT
from pretrain_curation_gym.val_set import (
...
def _fast_finalize_grace_period(monkeypatch):
    ...
    monkeypatch.setattr(CuratorTask, "FINALIZE_INTERVAL_SECONDS", 0.01)
...
class _Curator:
    ...
    async def prepared(self, state=None) -> dict:
        ...
        return await self.taskset.scorer().compute_scoring(trace.state, None)
...
def test_hf_token_is_validated_at_environment_load(monkeypatch):
...
def test_package_imports_without_torch_installed():
    ...
    ``import pretrain_curation_gym`` must succeed without torch on the path,
...
for name in list(sys.modules):
    ...
    if name == "pretrain_curation_gym" or name.startswith("pretrain_curation_gym."):
        del sys.modules[name]

import pretrain_curation_gym  # noqa: F401

from pretrain_curation_gym.gpu.train_gpt import steps_for_token_budget
...
def test_package_uses_the_installed_v1_without_path_mutation():
    import importlib
    import verifiers

    original_paths = list(verifiers.__path__)
    importlib.import_module("pretrain_curation_gym")
    importlib.import_module("verifiers.v1.env")
    assert list(verifiers.__path__) == original_paths
...
def test_load_environment_returns_v1_environment(monkeypatch):
    ...
    assert env.taskset.config.task.manifest_filename == MANIFEST_FILENAME
    assert env.harness.config.id == "default"
    assert env.harness.config.env == {}
    assert "HF_TOKEN" in env.harness.config.forward_env
...
def test_400m_eval_configs_set_train_microbatch_32(config_name):
    ...
    proxy = raw["taskset"]["task"]["curator"]["proxy_student"]
...
def test_load_environment_rejects_unknown_harness():
    with pytest.raises(ModuleNotFoundError, match="harness 'unknown' not found"):
...
def test_load_environment_injects_hf_token_into_harness_env(monkeypatch):
    ...
    assert "HF_TOKEN" in docker_env.harness.config.forward_env
    ...
    assert "HF_TOKEN" in subprocess_env.harness.config.forward_env
...
async def test_taskset_setup_fails_fast_when_hf_token_is_not_exported(monkeypatch):
    ...
    with pytest.raises(
        RuntimeError,
        match=r"PDC_TEST_HF_TOKEN.*required before starting a rollout",
    ):
        task = taskset.load()[0]
        trace = vf.Trace(
...
async def test_materialize_offloads_source_corpus_file_writes(monkeypatch):
    ...
    import pretrain_curation_gym.corpus as corpus_mod
    ...
    assert any(name.endswith("cache_documents") for name in offloaded)
    # Two sources → two raw cache writes + two scratch-file writes.
    assert sum(1 for name in offloaded if name.endswith("cache_documents")) == 2
...
async def test_materialize_propagates_store_docs_errors(monkeypatch):
    ...
    monkeypatch.setattr(CuratorState, "cache_documents", boom)
...
async def test_materialize_serializes_store_docs_state_writes(monkeypatch):
    ...
    real_store = CuratorState.cache_documents
    lock = threading.Lock()
    active = 0
    max_active = 0
    ...
    monkeypatch.setattr(CuratorState, "cache_documents", instrumented_store)
...
async def test_store_docs_cancellation_holds_lock_until_worker_done(monkeypatch):
    ...
    from pretrain_curation_gym.rollout_state import RolloutStore as _RS
...
async def test_from_docs_cancellation_holds_pipeline_sem(monkeypatch):
    ...
    from pretrain_curation_gym.corpus import SourceCorpus
...
def test_train_token_budget_equal_stages_400m_is_schedule_aware():
    ...
    from pretrain_curation_gym.gpu.train_gpt import scheduled_presentation_tokens
...
async def test_heavy_compute_is_offloaded(monkeypatch):
    import pretrain_curation_gym.rewards as rewards_mod
...
@pytest.mark.asyncio
async def test_reward_surface_has_only_perf_and_leakage():
    curator = _Curator()
    await _finalized(curator)
    trace = curator.trace()
    rewards = await discover_decorated(curator.taskset, "reward")[0](trace)
    assert set(rewards) == {"perf_reward", "leakage_penalty"}
...
def test_task_prompt_contract():
    ...
    assert task.data.system_prompt is None
    assert len(prompt) <= 6_300
    assert "complete freedom" in prompt
    assert '"token_budget": 1000000' in prompt
    assert '"sources"' in prompt
    assert "python self_score.py manifest.json" in prompt
...
def test_task_prompt_recurring_finalization_contract():
    ...
    assert "As soon as you have a viable candidate, create `manifest.json`" in prompt
    ...
    assert (
        ...
        "`manifest.json` a valid non-empty manifest" in prompt
    )
    ...
    assert "Creating `manifest.json` early does not end the episode" in prompt
...
def test_self_score_script_exposes_agent_controlled_cli_flags():
    ...
    import re as _re
    from pretrain_curation_gym.gpu import self_score as _self_score
...
async def test_default_taskset_path_builds_decon_detector():
    """The default task path builds its Decon detector in the scorer factory."""
    taskset = CuratorTaskset(
        CuratorTasksetConfig(
            id="test",
    ...
    assert scorer.decon_detector is not None
    assert scorer.decon_detector._binary == "/nonexistent/decon"
...
def test_reduce_hand_worked_example():
    ...
    from pretrain_curation_gym.leakage import DeconLeakageDetector
...
def test_decon_error_raised_on_decon_failure():
    ...
    from pretrain_curation_gym.leakage import DeconError, DeconLeakageDetector
...
def test_sandbox_script_is_the_written_train_gpt_source():
    ...
    from pretrain_curation_gym.trainer import TRAIN_GPT_PATH, _nanogpt_train_script
...
async def test_finalize_polls_for_late_workspace_manifest_file():
    ...
    async def write_manifest() -> None:
        await asyncio.sleep(curator.taskset.FINALIZE_INTERVAL_SECONDS * 2)
...
async def test_finalize_malformed_workspace_manifest_reports_invalid(
    ...
    assert (
        trace.info["manifest_provenance"] == MANIFEST_PROVENANCE_INVALID_WORKSPACE_FILE
    )
...
async def test_finalize_grace_period_picks_up_late_workspace_file():
    ...
    curator = await _make()
    trace = vf.Trace(
        task=vf.TraceTask(type=type(curator.task).__name__, data=curator.task.data),
        state=curator.state,
    )
    prompt = [vf.SystemMessage(content="sys"), vf.UserMessage(content="go")]
...
def test_turn_cap_is_disclosed_in_agent_prompt():
    ...
    assert (
        short[0].data.prompt.replace(
            "Turn limit: 7 model turns", "Turn limit: 200 model turns"
        )
        == long[0].data.prompt
    )
...
def test_task_prompt_renders_scoring_parameters_and_local_policy():
    ...
    assert (
        "Local sources are disabled; use only Hugging Face sources" in task.data.prompt
    )
    assert "`2.0 * performance - 3.0 * leakage`" in task.data.prompt
    assert f"{MANIFEST_FILENAME}" in task.data.prompt
    assert "read `.agents/skills/hf-cli/SKILL.md`" in task.data.prompt
...
def test_manifest_filename_is_configurable_and_rendered():
    ...
    assert env.taskset.config.task.manifest_filename == "curation-output.json"
    assert "curation-output.json" in task.data.prompt


@pytest.mark.parametrize("filename", ["/tmp/manifest.json"])
def test_manifest_filename_must_name_workspace_root_file(filename):
    with pytest.raises(ValidationError, match="runtime-workspace filename"):
...
def test_hf_cli_skill_is_packaged():
    ...
    assert (
        "pretrain_curation_gym/skills/hf-cli/**"
...
def test_docker_real_trainer_construction_does_not_import_modal():
    code = """
import sys
from pretrain_curation_gym.config import CuratorTaskConfig, CuratorTasksetConfig
from pretrain_curation_gym.taskset import CuratorTaskset

taskset = CuratorTaskset(CuratorTasksetConfig(
    id="test",
    task=CuratorTaskConfig(curator={
        "use_real_trainer": True,
        "proxy_student": {"runtime_backend": "docker"},
    }),
))
taskset.load()[0].scorer().trainer
assert "pretrain_curation_gym.modal_backend" not in sys.modules
"""
    subprocess.run(
        [sys.executable, "-c", code],
        check=True,
...
def test_val_build_eval_creates_valid_jsonl(tmp_path):
    ...
    from pretrain_curation_gym.leakage import DeconLeakageDetector, _VAL_EVAL_KEY
...
def test_val_leakage_detected_when_corpus_contains_val_text(tmp_path):
    ...
    """
    from pretrain_curation_gym.leakage import (
...
def test_val_eval_never_in_bundled_evals():
    ...
    """
    from pretrain_curation_gym.leakage import DEFAULT_EVAL_SETS_DIR
...
def test_screen_val_set_propagates_through_load_environment(monkeypatch):
    ...
    assert default_env.taskset.config.task.screen_val_set is True

    off_env = load_environment(screen_val_set=False)
    assert off_env.taskset.config.task.screen_val_set is False
...
def test_load_environment_accepts_perf_scaling_exponent(monkeypatch):
    ...
    assert env.taskset.load()[0].config.curator.perf_scaling_exponent == pytest.approx(
        1.5
    )
