...
def _make_temp_repo(tmp_path: Path) -> Path:
    ...
    cfg_src = REPO_ROOT / "environments" / "pretrain_curation_gym" / "configs" / "eval"
    cfg_dst = repo / "environments" / "pretrain_curation_gym" / "configs" / "eval"
    ...
    package_dst = repo / "environments" / "pretrain_curation_gym" / "scripts_400m"
...
def test_find_remote_results_strips_outputs_prefix(tmp_path: Path):
    ...
    (run_dir / "eval.log").write_text(
        "some preamble\nresults: outputs/pretrain-curation-gym--abc/uuid-123\n"
    )
    out = _run_find_results(body, tmp_path, "x")
    assert out.returncode == 0, out.stdout + out.stderr
    rel = out.stdout.strip()
    assert rel == "pretrain-curation-gym--abc/uuid-123"
...
def test_find_remote_results_fallback_returns_relative(tmp_path: Path):
    ...
    run = outputs_root / "pretrain-curation-gym--abc" / "run-uuid"
    ...
    assert out.returncode == 0, out.stdout + out.stderr
    rel = out.stdout.strip()
    assert rel == "pretrain-curation-gym--abc/run-uuid"
    assert not rel.startswith("outputs/")
...
def test_download_constructs_single_outputs_and_lands_local(tmp_path: Path):
    ...
    src = "root@203.0.113.9:/root/webcurator-gym/environments/pretrain_curation_gym/outputs/2026-01-01/run0/"
    assert src in args
