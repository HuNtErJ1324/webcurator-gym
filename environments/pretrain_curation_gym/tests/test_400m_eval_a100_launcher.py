...
def _make_temp_repo(tmp_path: Path) -> Path:
    ...
    (repo / "environments").symlink_to(
        REPO_ROOT / "environments", target_is_directory=True
    )
...
def test_launcher_tracked_eval_configs_pin_train_microbatch_32():
    ...
    env_dir = REPO_ROOT / "environments" / "pretrain_curation_gym"
    for rel in rels:
        path = env_dir / rel
...
def test_codex_patch_generated_launch_args_disable_both_keep_function_tools(
    tmp_path: Path,
):
    ...
    start = next(
        ...
        if "PRIME_INFERENCE_DISABLE_NAMESPACE_TOOLS" in ln
    )
    end = next(
        j
    ...
    ns = {
        "self": types.SimpleNamespace(config=types.SimpleNamespace(disabled_tools=None))
    }
...
def test_codex_patch_fails_fast_real_script_no_eval(tmp_path: Path):
    ...
    proc = subprocess.run(
        [
            str(repo / "scripts" / EVAL_SCRIPT.name),
            "--model",
            "z-ai/glm-5.2",
            "--skip-site",
        ],
    ...
    assert not (record_dir / "eval_ran.log").exists(), (
        "eval must not run if patch fails"
    )
