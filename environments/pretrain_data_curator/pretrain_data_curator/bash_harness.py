...
Imports of ``BashHarness`` are deferred so constructing this module never fails
...
def truncating_program_source() -> str:
    ...
    if _PROGRAM_SOURCE is None:
        from verifiers.v1.harnesses.bash.harness import PROGRAM_SOURCE as stock
...
def get_truncating_bash_harness_class() -> type[Any]:
    """Return the TruncatingBashHarness class, importing BashHarness lazily."""
    ...
    class TruncatingBashHarness(BashHarness):
        """Bash harness that caps tool results via ``MAX_TOOL_OUTPUT_CHARS``."""
        ...
        async def launch(
            ...
            if isinstance(prompt, str):
                ...
                env["INITIAL_MESSAGES"] = json.dumps(
                    [message_to_wire(m) for m in prompt]
                )
