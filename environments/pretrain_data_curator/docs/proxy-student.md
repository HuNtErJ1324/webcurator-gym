...
Real training requires `proxy_student.runtime_backend` to be either `"docker"`
or `"modal"`. The field is a static hint used to build the harness runtime
before a rollout starts. At score time, `RuntimeSelectedTrainer` dispatches from
the live runtime's type, so the configured backend and actual harness runtime
must agree. A separate Prime GPU sandbox backend is not supported.

## Backend: Docker harness runtime

`runtime_backend="docker"` changes the v1 harness runtime itself. Discovery,
...
`runtime_backend="modal"` also runs the complete rollout in one GPU runtime.
...
[`../Dockerfile.runtime`](../Dockerfile.runtime) is the reference image
contract. It combines bash/`hf` discovery with CUDA PyTorch, tokenizer
dependencies, and the generated trainer requirements. Modal pulls an already
published registry image; it does not build the local Dockerfile.
