...
5. If no manifest was emitted, a last-resort trace recovery can synthesize
   sources from dataset IDs seen in `hf datasets info` commands or tool output.
...
| `doc_cache` | Fetched document slices keyed by a stable `FetchKey` |
...
fetches, so the Hub call and token charge occur once.
