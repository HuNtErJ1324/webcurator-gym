...
| Field | Default | Notes |
| --- | --- | --- |
| `trainer_backend` | `"prime"` | `"prime"`, `"docker"`, or `"modal"` |
| `docker_image` | Prime PyTorch 2.3.1 image | Docker/Modal validators replace the untouched default with PyTorch 2.7 CUDA 12.6 |
...
| `gpu_type` | `"H100"` | Prime-only GPU type |
| `vm` | `true` | Prime requires VM mode when requesting a GPU |
...
| `create_timeout_seconds` | `300.0` | Prime sandbox creation timeout |
| `upload_timeout_seconds` | `120.0` | Prime upload timeout |
...
floor. Prime and Modal cap derived/explicit timeouts at 24 hours; self-hosted
Docker does not.
...
## Prime sandbox real trainer

Prime is the default `trainer_backend`, so the smallest real-trainer override is:

```bash
prime eval run pretrain-data-curator \
  -m openai/gpt-4.1-mini -n 1 -r 1 \
  -a '{"use_real_trainer": true, "proxy_student": {"train_token_budget": 5000000}}'
```

This path provisions a separate `prime_sandboxes` GPU sandbox during scoring.
The ordinary harness runtime remains unchanged. A positive `gpu_count` requires
`vm=true` and a nonempty `gpu_type`. Install the package's `sandbox` extra in
the environment worker before using this backend:
...
[`../configs/eval/full-reference.toml`](../configs/eval/full-reference.toml)
