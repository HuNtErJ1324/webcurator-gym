...
def _run(tmp_path, *, avail, status, rsync_mode="ok"):
    ...
    return _run_script(tmp_path, repo, avail=avail, status=status, rsync_mode=rsync_mode)
...
def test_non_root_fails_before_sync(tmp_path):
    ...
    assert not (record / "chmod.log").exists(), "secrets must not be uploaded for non-root"
    assert result.returncode != 0
...
def test_permanent_rsync_fails_immediately(tmp_path):
    result, record = _run(tmp_path, avail=_AVAIL_MIXED, status=_STATUS_ROOT, rsync_mode="permanent")
...
def test_secrets_mode_enforced(tmp_path):
    ...
    assert chmod_log.exists(), f"remote chmod on secrets.env never ran; stderr={result.stderr}"
