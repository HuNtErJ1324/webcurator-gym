...
from pretrain_data_curator.runtime_config import (
...
def test_materialize_local_docs_offload_matches_sync_parse(fmt: str, tmp_path: Path):
    ...
    async def run() -> list[str]:
        return await asyncio.to_thread(
            _materialize_local_docs, raw_path, raw, fmt, "text" if fmt == "jsonl" else None
        )

    offloaded = asyncio.run(run())
    # The offloaded parse must equal the synchronous parse of the same file.
    raw_path.write_text(raw, encoding="utf-8")
    expected = list(_iter_local_documents(raw_path, fmt, "text" if fmt == "jsonl" else None))
...
def test_derive_task_runtime_updates_default_is_empty():
    assert derive_task_runtime_updates(ProxyStudentConfig(), use_real_trainer=False) == {}
    # A real trainer without a backend (None) also yields no updates.
    assert derive_task_runtime_updates(ProxyStudentConfig(), use_real_trainer=True) == {}
