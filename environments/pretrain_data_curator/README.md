...
- `pretrain_data_curator.py` — `load_environment` entry point; builds `CuratorTasksetConfig` and returns a native v1 `Environment`.
- `truncating_environment.py` — `Environment`: thin native v1 `vf.Environment` subclass that caps Codex tool results at the interception→provider wire boundary (the only behavior retained from the old v0 compatibility façade).
- `truncating_client.py` — `TruncatingClient` / `wrap_client`: delegate to an inner client after capping tool outputs on the request `body`.
